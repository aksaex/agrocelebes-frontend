...
export default function Login() {
  ...
  // State untuk mengatur kapan tombol muncul (Awalnya false)
  const [showInstallBtn, setShowInstallBtn] = useState(false);

  useEffect(() => {
    // Chrome akan menembakkan event ini JIKA aplikasi memenuhi syarat PWA
    // dan JIKA aplikasi belum diinstal.
    const handleBeforeInstallPrompt = (e) => {
      e.preventDefault();
      setDeferredPrompt(e);
      setShowInstallBtn(true); // Chrome bilang OK, kita munculkan tombolnya!
    };

    window.addEventListener('beforeinstallprompt', handleBeforeInstallPrompt);

    // Cek juga apakah aplikasi sudah terinstal (berjalan di mode standalone)
    // Jika iya, jangan munculkan tombol.
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      // Tampilkan pop-up install bawaan Chrome/Android
      deferredPrompt.prompt();
      
      // Tunggu respon pengguna (Accept / Dismiss)
      const { outcome } = await deferredPrompt.userChoice;
      if (outcome === 'accepted') {
        toast.success('Aplikasi AgroCelebes berhasil diinstal! ✅', { duration: 4000 });
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2rem] shadow-2xl w-full max-w-4xl border border-gray-100 flex flex-col md:flex-row gap-8 lg:gap-16 items-center">
        ...
        <div className="w-full md:w-1/2 text-center md:text-left flex flex-col items-center md:items-start relative">
          <div className="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center mb-6 shadow-inner border border-green-100 relative">
            ...
            {/* Animasi Ping hanya muncul jika tombol install aktif */}
            {showInstallBtn && (
          ...
          {showInstallBtn && (
            <button 
              ...
              className="mb-8 w-full md:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-primary text-white px-5 py-3 rounded-xl font-bold shadow-lg shadow-green-500/30 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 animate-fade-in"
            >
        ...
        <div className="w-full md:w-1/2 bg-gray-50/50 p-2 rounded-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div className="flex items-center bg-white border border-gray-200 rounded-xl p-3 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition">
              ...
              <input type="email" placeholder="Email Terdaftar" required value={email} onChange={(e) => setEmail(e.target.value)} className="bg-transparent outline-none w-full text-sm" />
            </div>

            <div className="flex items-center bg-white border border-gray-200 rounded-xl p-3 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition">
              <Lock size={20} className="text-gray-400 mr-3" />
              <input type="password" placeholder="Password" required value={password} onChange={(e) => setPassword(e.target.value)} className="bg-transparent outline-none w-full text-sm" />
            </div>

            <div className="flex justify-end mt-[-10px]">
              <Link to="/forgot-password" className="text-sm text-red-500 font-semibold hover:underline">Lupa Sandi?</Link>
            </div>

            <button type="submit" disabled={loading} className="w-full mt-2 bg-primary text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-green-700 transition disabled:bg-gray-400 shadow-md group">
